...
func newOpenPublicForm(t *testing.T, um mwanachamaforms.FormManager, key string) mwanachamaforms.Form {
	t.Helper()
	f, err := um.Create(context.Background(), mwanachamaforms.Form{
		Title: "Census", OriginatorChapterID: "chapter-1", ClosesAt: futureRFC3339(time.Hour), Audience: mwanachamaforms.AudiencePublic,
	})
	if err != nil {
...
func TestUpsertRespondent(t *testing.T) {
	um := newTestManager(t)
	first, err := um.UpsertRespondent(context.Background(), mwanachamaforms.Respondent{PublicKey: "pk-1"})
	if err != nil {
	...
	second, err := um.UpsertRespondent(context.Background(), mwanachamaforms.Respondent{PublicKey: "pk-1"})
	if err != nil {
...
func TestSubmitAnswers(t *testing.T) {
	...
	yes := true
	out, err := um.SubmitAnswers(context.Background(), f.ID, "member-1", "chapter-2", []mwanachamaforms.Answer{
	...
	no := false
	out2, err := um.SubmitAnswers(context.Background(), f.ID, "member-1", "chapter-3", []mwanachamaforms.Answer{
...
func TestSubmitAnswers_FormNotOpen(t *testing.T) {
	...
	_, err = um.SubmitAnswers(context.Background(), f.ID, "member-1", "chapter-2", []mwanachamaforms.Answer{{QuestionID: q.ID, ValueText: "hi"}})
...
func TestSubmitAnswers_InvalidShape(t *testing.T) {
	...
	_, err = um.SubmitAnswers(context.Background(), f.ID, "member-1", "chapter-2", []mwanachamaforms.Answer{{QuestionID: q.ID}})
...
func TestDeclare(t *testing.T) {
	...
	r, err := um.UpsertRespondent(context.Background(), mwanachamaforms.Respondent{PublicKey: "pk-2"})
	if err != nil {
		t.Fatalf("UpsertRespondent: %v", err)
	}
	d, err := um.Declare(context.Background(), mwanachamaforms.Declaration{RespondentID: r.ID, FormID: f.ID, DeclaredChapterID: "chapter-9"})
	if err != nil {
	...
	updated, err := um.Declare(context.Background(), mwanachamaforms.Declaration{RespondentID: r.ID, FormID: f.ID, DeclaredChapterID: "chapter-10"})
	if err != nil {
